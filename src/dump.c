...
static void jl_serialize_code_instance(jl_serializer_state *s, jl_code_instance_t *codeinst, int skip_partial_opaque, int oc_hack) JL_GC_DISABLED
{
    if (oc_hack && jl_serialize_generic(s, (jl_value_t*)codeinst)) {
        return;
    }
    ...
    if (write_ret_type && codeinst->rettype_const &&
        ...
        if (skip_partial_opaque) {
            jl_serialize_code_instance(s, codeinst->next, skip_partial_opaque, 1);
            return;
        }
        else {
...
static void jl_serialize_value_(jl_serializer_state *s, jl_value_t *v, int as_literal) JL_GC_DISABLED
{
    ...
    if (jl_is_svec(v)) {
        ...
        jl_serialize_code_instance(s, mi->cache, 1, 1);
    }
    else if (jl_is_code_instance(v)) {
        jl_serialize_code_instance(s, (jl_code_instance_t*)v, 0, 0);
    }
...
static void collect_backedges(jl_method_instance_t *callee) JL_GC_DISABLED
{
    ...
    if (backedges) {
        ...
        while (i < l) {
            ...
            if (*edges == HT_NOTFOUND)
                *edges = jl_alloc_vec_any(0);
            jl_array_ptr_1d_push(*edges, (jl_value_t*)invokeTypes);
    ...
    if (callees != HT_NOTFOUND) {
        ...
        for (i = 1; i < l; i += 2) {
            ...
            if (c && jl_is_method_instance(c)) {
                assert(!module_in_worklist(c->def.method->module)); // these are preserved already as backedges if usable and applicable
                arraylist_push(wq, c);
            }
        }
    }
...
static void jl_collect_edges(jl_array_t *edges, jl_array_t *ext_targets)
{
    ...
    for (size_t i = 0; i < wq.len; i++) {
        jl_method_instance_t *caller = (jl_method_instance_t*)wq.items[i];
        jl_record_edges(caller, &wq, edges);
    }
    arraylist_free(&wq);
    ...
    for (size_t i = 0; i < l; i += 2) {
        ...
        for (size_t j = 0; j < l; j += 2) {
            ...
            idxs[j / 2 + 1] = (char*)target - (char*)HT_NOTFOUND - 1;
        }
        if (valid)
            jl_array_ptr_set(edges, i + 1, callee_ids); // swap callees for ids
        else
            jl_array_ptr_set(edges, i + 1, NULL);
    }
    // record place of every method in edges
    htable_reset(&edges_map, 0);
    for (size_t i = 0; i < l; i += 2) {
        jl_method_instance_t *caller = (jl_method_instance_t*)jl_array_ptr_ref(edges, i);
        void *target = (void*)((char*)HT_NOTFOUND + i / 2 + 1);
        ptrhash_put(&edges_map, (void*)caller, target);
    }
    // add method edges to the callee_id list
    for (size_t i = 0; i < l; i += 2) {
        jl_array_t *callee_ids = (jl_array_t*)jl_array_ptr_ref(edges, i + 1);
        if (!callee_ids)
            continue; // invalid edges
        int32_t *idxs = (int32_t*)jl_array_data(callee_ids);
        size_t l = idxs[0];
        size_t nt = l + 1;
        for (size_t j = 0; j < l; j++) {
            ...
            if (callee && jl_is_method_instance(callee)) {
                void *target = ptrhash_get(&edges_map, (void*)callee);
                if (target != HT_NOTFOUND) {
                    idxs[nt++] = (char*)target - (char*)HT_NOTFOUND - 1;
                }
            }
        }
        jl_array_del_end(callee_ids, 2 * l + 1 - nt);
    }
    htable_reset(&edges_map, 0);
}
...
static jl_array_t *jl_verify_edges(jl_array_t *targets)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (invokesig) {
            ...
            jl_value_t *matches = jl_gf_invoke_lookup_worlds(invokesig, (jl_value_t*)mt, world, &min_valid, &max_valid);
            if (matches == jl_nothing) {
                 valid = 0;
                 break;
            }
            ...
            jl_value_t *matches = jl_matching_methods((jl_tupletype_t*)sig, jl_nothing,
                    -1, 0, world, &min_valid, &max_valid, &ambig);
...
void jl_verify_methods(jl_array_t *edges, jl_array_t *valids, htable_t *visited) JL_NOTSAFEPOINT
{
...
static void mark_edges_in_worklist(jl_array_t *edges, int idx, htable_t *visited, int found) JL_NOTSAFEPOINT
{
    jl_method_instance_t *caller = (jl_method_instance_t*)jl_array_ptr_ref(edges, idx);
    ...
    jl_array_t *callee_ids = (jl_array_t*)jl_array_ptr_ref(edges, idx + 1);
    assert(jl_typeis((jl_value_t*)callee_ids, jl_array_int32_type));
    int32_t *idxs = (int32_t*)jl_array_data(callee_ids);
    size_t i, n = jl_array_len(callee_ids);
    for (i = idxs[0] + 1; i < n; i++) {
        int32_t idx = idxs[i];
        mark_edges_in_worklist(edges, idx, visited, found);
    }
}


// Visit the entire call graph, starting from edges[idx] to determine if that method is valid
static int jl_verify_graph_edge(jl_array_t *edges, int idx, htable_t *visited, int depth) JL_NOTSAFEPOINT
{
...
static void jl_insert_backedges(jl_array_t *edges, jl_array_t *ext_targets, jl_array_t *mi_list)
{
    ...
    // next enable any applicable codes
    for (i = 0; i < l; i++) {
        jl_method_instance_t *caller = (jl_method_instance_t*)jl_array_ptr_ref(edges, 2 * i);
        ...
        if (ci != HT_NOTFOUND) {
            ...
            // TODO: supposed to be only internal here, but may be internal or external (this will normally correctly ignore external things however)
    ...
    // finally disable any invalid codes
    // n.b. This may disable code we just enabled in the rare case there was a backedge added
    // for an external method whose edges loop through this new module.
    // TODO: should we pre-compute those edges and add them to the set at the boundary,
    //       so we don't attempt to add them, then immediately delete them/ again?
    for (i = 0; i < l; i++) {
        jl_method_instance_t *caller = (jl_method_instance_t*)jl_array_ptr_ref(edges, 2 * i);
        assert(jl_is_method_instance(caller) && jl_is_method(caller->def.method));
        int valid = jl_array_uint8_ref(valids, i);
